<?php
/*
Plugin Name: WTG Custom Permalinks
Plugin URI: http://wtg.co.uk
Description: Set custom permalinks on a per-post basis. This is a fork of the Enhanced Custom Permalink plugin which is
full of errors and deprecated functions.
Version: 0.1
...
function custom_permalinks_content_list_verify($query,$thisPostID){
	...
	$sql = "SELECT $wpdb->posts.ID, $wpdb->postmeta.meta_value, $wpdb->posts.post_type FROM $wpdb->posts ".
				...
				"  meta_value != '' AND ".
				"  (LOWER(meta_value) = LOWER('".mysql_real_escape_string($request_noslash)."') OR ".
				"	LOWER(meta_value) = LOWER('".mysql_real_escape_string($request_noslash."/")."') ) ".
...
function custom_permalinks_request($query) {
	...
	if(isset($url['path']) && intval($_GET['preview_id'])!=0)
	{
		$forcematch = " AND $wpdb->posts.ID = ".intval($_GET['preview_id'])." ";   
	}
...
function custom_permalinks_term_options($object) {
	$permalink = custom_permalinks_permalink_for_term(isset($object->term_id) ? $object->term_id : FALSE);

	$originalPermalink = FALSE;
...
function custom_permalinks_setup_admin() {
	add_management_page( 'Custom Permalinks', 'Custom Permalinks', 'manage_options', 'custom_permalinks',
		'custom_permalinks_options_page' );
	if ( is_admin() )
	{
